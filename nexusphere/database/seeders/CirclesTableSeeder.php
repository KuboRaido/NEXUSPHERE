<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Carbon;

class CirclesTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        DB::table('circles')->insert([
            [
                'circle_id'   => 1,
                'circle_name' => 'AI研究部',
                'category'    => '研究',
                'owner_id'    => 2,
                'sentence'    => 'AIに関する研究を行うサークルです。',
                'icon'        => 'ai_circle.png',
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
            [
                'circle_id'   => 2,
                'circle_name' => '音楽制作サークル',
                'category'    => '音楽',
                'owner_id'    => 2,
                'sentence'    => '音楽制作に関するサークルです。',
                'icon'        => 'music_circle.png',
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
            [
                'circle_id'   => 3,
                'circle_name' => 'ゲーム開発同好会',
                'category'    => 'ゲーム',
                'owner_id'    => 2,
                'sentence'    => 'ゲーム開発に興味のある人の集まりです。',
                'icon'        => 'game_circle.png',
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
            [
                'circle_id'   => 4,
                'circle_name' => '写真部',
                'category'    => 'アート',
                'owner_id'    => 2,
                'sentence'    => '風景やポートレートを撮り歩き、作品を共有するサークルです。',
                'icon'        => null,
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
            [
                'circle_id'   => 5,
                'circle_name' => '英会話クラブ',
                'category'    => '語学',
                'owner_id'    => 2,
                'sentence'    => 'ネイティブ講師とともに日常英会話を楽しく学ぶサークルです。',
                'icon'        => null,
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
            [
                'circle_id'   => 6,
                'circle_name' => '登山サークル',
                'category'    => 'アウトドア',
                'owner_id'    => 2,
                'sentence'    => '週末に近郊の山へ登り、自然を満喫するアウトドアサークルです。',
                'icon'        => null,
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
            [
                'circle_id'   => 7,
                'circle_name' => '料理研究会',
                'category'    => '食',
                'owner_id'    => 2,
                'sentence'    => '世界各国のレシピに挑戦し、みんなで作って味わうサークルです。',
                'icon'        => null,
                'created_at' => Carbon::now(),
                'updated_at' => Carbon::now(),
            ],
        ]);
    }
}
