...
async function loadRequestList(){
  const listRoot = document.getElementById('requestList');
  if(!listRoot) return;

  listRoot.innerHTML = '<li class="loading">読み込み中...</li>';

  try{
    const res = await fetch('/api/v1/circle/{circle}/request',{
      headers: {'Accept': 'application/json'},
      credentials: 'include',
    });
    const json = await res.json();
    if (!res.ok) throw new Error(json.message|| '申請一覧の取得に失敗しました');

    const items = Array.isArray(json) ? json : (json.data ?? json.requests ?? []);
    if(!items.length){
      listRoot.innerHTML = '<li class="empty">まだ申請がありません</li>';
      return;
    }
